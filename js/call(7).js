  ...
  function handleMessage(msg) {
    switch (msg.type) {
      ...
      case 'customer_detected':
        // Show name + predicted reason in call panel immediately
        showCustomer(msg.customer);
        // Open their report RIGHT NOW while they're still talking
        if (window._siyaqOpenReport && msg.customer?.id) {
          window._siyaqOpenReport(String(msg.customer.id));
        }
        // Move call panel so it doesn't cover the report
      ...
      case 'call_ended':
        ...
        // Reload customers so the new interaction is visible immediately,
        // then refresh the open report panel if one is showing
        if (window._siyaqLoadCustomers) {
          window._siyaqLoadCustomers().then(() => {
            if (window._siyaqRefreshOpenReport) {
              window._siyaqRefreshOpenReport();
            }
          }).catch(() => {});
        }
